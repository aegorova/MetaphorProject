...
if ($_POST["assignmentId"] == ""){
	...
	$results = explode(";", $_POST["results"]);
  	$subjectId = $theFileName;
	echo($subjectId);
  	echo("<form method='POST' action='http://mturk.com/mturk/externalSubmit'><input type='hidden' name='answers' value='" . $post . "'><input type='hidden' name='assignmentId' value='" . $subjectId . "'><input id='submitButton' type='submit' name='Submit' value='Submit'></form>");
}
